...
class HanaIFScraper:
    def __init__(self):
        ...
        self.press_release_url = f"{self.base_url}/boardList.do?menuId=MN1000&tabMenuId=N"
    ...
    def parse_releases(self, html):
        ...
        items = soup.select('.listType01 li')
        total_items = len(items)
        print(f"총 {total_items}개의 항목이 발견되었습니다. 처리 중입니다...")

        for idx, item in enumerate(items):
            date = item.select_one('.sourceBox .date').get_text(strip=True)
            if date == yesterday:
                title_element = item.select_one('.tit')
                title = title_element.get_text(strip=True)
                link = self.base_url + item.select_one('a.txtBox')['href']
                releases.append({
                    'title': title,
                    'link': link,
                    'date': date,
    ...
    def scrape(self):
        ...
        if releases:
            save_to_csv(releases, "hanaif_press_releases.csv", self.output_dir)
            print(f"어제 날짜의 보도자료 {len(releases)}개를 찾았습니다.")
        else:
            print("어제 날짜의 보도자료를 찾지 못했습니다.")

        # 스크래핑 완료 메시지 출력 (보라색 텍스트)
        print("\033[95m하나금융연구소 스크래핑이 완료되었습니다\033[0m")
